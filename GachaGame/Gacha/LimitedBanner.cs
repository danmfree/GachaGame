...
namespace GachaGame.Gacha
{
    public class LimitedBanner
    {
        ...
        public LimitedBanner(PlayerData playerData)
        {
        ...
        {
            playerData.LimitedPity5++;
            playerData.LimitedPity4++;

            double fiveStarChance = 0.006; // 0,006

            if (playerData.LimitedPity5 >= SoftPity5Start)
            {
                int stepsIntoSoft = playerData.LimitedPity5 - SoftPity5Start + 1;
                fiveStarChance = Math.Min(1.0, 0.006 + stepsIntoSoft * 0.15);
            }

            if (playerData.LimitedPity5 >= HardPity5)
                fiveStarChance = 1.0;
            ...
            if (got5)
            {
                ...
                if (playerData.LimitedGuaranteedFeatured)
                {
                    baseChar = FeaturedFiveStar;
                    playerData.LimitedGuaranteedFeatured = false;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        playerData.LimitedGuaranteedFeatured = true;
                    }
                }

                Character c = new Character(baseChar.Name, baseChar.Rarity)
                {
                    PulledAtPity5 = playerData.LimitedPity5
                };

                playerData.LimitedPity5 = 0;
                playerData.LimitedPity4 = 0;

                return c;
            }

            double fourStarChance = 0.051;
            bool guaranteed4 = playerData.LimitedPity4 >= HardPity4;
            ...
            if (got4)
            {
                ...
                c.PulledAtPity4 = playerData.LimitedPity4;
